...
//  all triworkouts in a sessionlog is greater than the set distance of the monitor
...
namespace P5
{
    public class UndertrainingMonitor : IObserver<SessionLog>
    {
        private string Name;
        private int Target;
        ...
        public UndertrainingMonitor(string name, int target, TextWriter writer)
        {
            ...
            foreach (Workout work in temp)
            {
                ...
                if (totalPerformed < Target && !check)
                {
                    ...
                    Writer.Write("The combined performed distance of all TriWorkouts in the given session log\n" +
                                    "is less than the target distance of the Monitor\n\n");
                }
            }
            Writer.Flush();
        }
